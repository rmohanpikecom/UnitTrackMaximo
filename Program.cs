    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                Writer.WriteLine(DateTime.Now.ToString("yyyyMMddHHmmss") + "DynamicsPikeService - " + AppName + " - Started");
        ...
        public static void GetWorkorders_DYN()
        {
            Console.WriteLine("DynamicsPikeService - " + AppName + " - Started");
            FileStream? stream = null;
            // File name  
            ...
            if (dsProjectTask.Tables[0].Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dsProjectTask.Tables[0].Rows.Count; i++)
                {
                    ...
                    try
                    {
                        Console.WriteLine("DynamicsPikeService - " + AppName + " - Total ProjectTask " + dsProjectTask.Tables[0].Rows.Count.ToString());
                        writer.WriteLine("DynamicsPikeService - " + AppName + " - Total ProjectTask " + dsProjectTask.Tables[0].Rows.Count.ToString());

                        string CurrentRun = DateTime.Now.ToString("yyyy-MM-ddTHH\\:mm\\:ss.fffzzz");
                        try
                        {
                            Console.WriteLine("Processing ProjectTask Records - " + i.ToString() + " - out of " + dsProjectTask.Tables[0].Rows.Count.ToString());
                            writer.WriteLine("Processing ProjectTasks Records - " + i.ToString() + " - out of " + dsProjectTask.Tables[0].Rows.Count.ToString());

                            TaskNumber = dsProjectTask.Tables[0].Rows[i]["hsl_workorderid"].ToString()!;
                            Task_Id = dsProjectTask.Tables[0].Rows[i]["hsl_oracletaskid"].ToString()!;
                            Project_Number = dsProjectTask.Tables[0].Rows[i]["hsl_oracleprojectnumber"].ToString()!;
                            Project_Id = dsProjectTask.Tables[0].Rows[i]["hsl_oracleid"].ToString()!;
                            BusinessUnitName = dsProjectTask.Tables[0].Rows[i]["hsl_businessunitname"].ToString()!;
                            ProcessDate = DateTime.Now.ToString("yyyy-MM-dd");

                            int res = clsDAL.SQL_ProjectTask_Create(TaskNumber, Task_Id, Project_Number, Project_Id, BusinessUnitName, ProcessDate);

                            Console.WriteLine("DynamicsPikeService - " + AppName + " - ProjectTask Completed");
                            writer.WriteLine("=========================================================================");
        ...
        #region GetDukeWorkorders
        public static void GetDukeWorkorders()
        {
            try
            {
                ...
                DataSet ds = clsDAL.SQL_ProjectTask_GetList();

                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    try
                    {
                        Console.WriteLine(DateTime.Now.ToString("yyyy-MM-ddTHH\\_mm") + AppName + " - Started " + i);
                        writer.WriteLine(DateTime.Now.ToString("yyyy-MM-ddTHH\\_mm") + AppName + " - Started " + i);

                        Console.WriteLine(AppName + "- Started :" + DateTime.Now.ToString("yyyy-MM-ddTHH\\_mm") + i);
                        writer.WriteLine(AppName + "- Started :" + DateTime.Now.ToString("yyyy-MM-ddTHH\\_mm") + i);
                        ...
                        GetDukeCompatibleUnits(WorkOrder_Id, WorkOrder_Number, i, ds.Tables[0].Rows.Count);
                      
                    }
                    catch (Exception exp)
                    {
                        writer.WriteLine("UnitTrack Maximo Update Failure");
        ...
        public static void GetDukeProjectTaskData()
        {
            ...
            string @PARENT_TASKNO = "10012889";
            string @SUBTASK = "44107-10012889";
            string @SERVICE_ITEM = "OAA01-MAN HOUR RATE PER MAN OH";
            string @Quantity = "15";
            string @EffectiveDate = "2023-01-22";
        ...
        public static string GetDukeCompatibleUnits(int WorkOrder_Id, string WorkOrder_Number, int WO_Ctr, int WO_Count)
        {
            ...
            string? fileName = filePath + FolderName + "\\UnitTrack_Log_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".txt";
            stream = new FileStream(fileName, FileMode.OpenOrCreate);
            // Create a StreamWriter from FileStream  
            StreamWriter writer = new StreamWriter(stream, Encoding.UTF8);
            writer.AutoFlush = true;
                    ...
                    if (dyArray.responseList.Count != null)
                    {
                        for (int i = 0; i < dyArray.responseList.Count; i++)
                        {
                            try
                            {
                                ...
                                Console.WriteLine("");
                                Console.WriteLine(AppName + "- Work Order :" + (WO_Ctr+1).ToString() + " of " + WO_Count.ToString() + " and Record :" + Count.ToString()+" of "+ RecordCount.ToString());
